...
void Image::Render(ID2D1BitmapRenderTarget* renderTarget,const Vector2& position)
{
	Vector2 size = mSize * mScale;
	...
	renderTarget->SetTransform(scaleMatrix * rotateMatrix * translateMatrix);
	renderTarget->DrawBitmap(mBitmap, dxArea, mAlpha);
	ResetRenderOption();
}
