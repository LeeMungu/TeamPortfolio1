...
void WorkTable::Init()
{
	...
	mMakeWoodBoard = new Button(L"나무판", mX + 230, mY + 120, 80, 50, [this]() {mBtnKey = "WoodBoard"; MakingCountSet(); DeleteItem(); Worktemplet(mBtnKey); });
	//vmMakeBonFire = new Button(L"모닥불", mX + 230, mY + 140, 80, 50, [this]() {mBtnKey = "BonFire", Worktemplet(mBtnKey); });
	mMakeBarrigate = new Button(L"바리게이트", mX + 230, mY + 160, 80, 50, [this]() {mBtnKey = "Barrigate"; MakingCountSet(); DeleteItem(); Worktemplet(mBtnKey); });
	mMakeBoom = new Button(L"폭탄", mX + 230, mY + 200, 80, 50, [this]() {mBtnKey = "Boom"; MakingCountSet(); DeleteItem(); Worktemplet(mBtnKey); });
				...
				for (int i = 0; i < items.size(); ++i) {
					if (((Item*)items[i])->GetItemKind() == ItemKind::inventory)
					{
						if (((Item*)items[i])->GetKeyName() == L"WoodBrench1")
						{
							...
							mMakeCost = 2;
							//재료아이템
							Item* workTableitem = new Item(((Item*)items[i])->GetKeyName(), "wood", mX + 450, mY + 250, ((Item*)items[i])->GetCount(), ItemKind::holding);
							workTableitem->Init();
			...
			if (items.size() != NULL)
			{
				for (int i = 0; i < items.size(); ++i)
				{
					if (((Item*)items[i])->GetItemKind() == ItemKind::inventory)
					{
						...
						{
							//재료아이템
							Item* workTableitem = new Item(((Item*)items[i])->GetKeyName(), "WoodBoard", mX + 450, mY + 250, ((Item*)items[i])->GetCount(), ItemKind::holding);
							workTableitem->Init();
							ObjectManager::GetInstance()->AddObject(ObjectLayer::MakingItem, workTableitem);
			...
			if (items.size() != NULL)
			{
				for (int i = 0; i < items.size(); ++i)
				{
					if (((Item*)items[i])->GetItemKind() == ItemKind::inventory)
					{
						...
						{
							//재료아이템
							Item* workTableitem = new Item(((Item*)items[i])->GetKeyName(), "Iron1", mX + 450, mY + 250, ((Item*)items[i])->GetCount(), ItemKind::holding);
							workTableitem->Init();
							ObjectManager::GetInstance()->AddObject(ObjectLayer::MakingItem, workTableitem);
							mMakeItemCount = ((Item*)items[i])->GetCount();
						}
